...
def _populate_quadrants(quad1):
    return reduce(lambda x, p: x + [(p[0], p[1]), (-p[0], p[1]), (p[0], -p[1]), (-p[0], -p[1])], quad1, [])
...
def _hex_diamond_thing_indicies(half_height=3, half_width=3):
    ...
    """
    return _populate_quadrants([(r,c) for r in range(half_height) for c in range(half_width) if r+c < (half_width+half_height)/2])

def _slope_indices(height, width):
    slope = 1.0*height/width
    indicies = filter(lambda x: x[0] <= (width-x[1])*slope,
